...
#' @examples
#' \dontrun{
#' step_plt(dataframe = df, varname = "Etr",type="line",obs=TRUE)
#' }
#'
#'
#'
step_plt = function(dataframe,
                     varname,
                     unit=NULL,
                     type="line",
                     obs=F,a,b){
  ...
  if (type=="point"){
    ...
    if (obs == TRUE){
      ...
      if (sum(is.na(dataframe[2])) < nrow(dataframe[3])/1.5){
        ...
        df$Rainfall <- a-df$Rain/b

        df2 <- dataframe %>%
          select(Date,all_of(var),Rainfall) %>%
          gather(key = "variable", value = "value", -Date)

        ggplot(df2, aes(x = ymd(Date), y=value)) +

          geom_point(data=subset(df2,variable == varname),size = 0.5)
          geom_point(data=subset(df2,variable == paste0(varname,".simu")),size = 0.5)
          geom_line(data=subset(df2,variable == "Rainfall",linetype = "solid"), size = 0.5)+
          scale_color_manual(labels = c("Rain","Observation","Simulation"),
                             values = c("blue","black","peru")) +
          scale_y_continuous(sec.axis=sec_axis(~(a-.)*b, name= "Rain (mm)"))+
          scale_x_date(date_labels="%Y",date_breaks  ="1 year",expand = c(0, 0))+
